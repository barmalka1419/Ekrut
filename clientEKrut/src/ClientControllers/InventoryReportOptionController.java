...
/**
 * The Class InventoryReportOptionController is controller for the screen InventoryReportOption.fxml.
 * This class is setup the screen at first and than gives the CEO or to the Regional Manager options to choose specific
 * report to look at
 */
public class InventoryReportOptionController {
    ...
    ObservableList<String> Machine=FXCollections.observableArrayList("1", "2","3","4","5","6");
    comboBoxYear.setItems(Year);
    comboBoxMonth.setItems(Month);
    comboBoxMachine.setItems(Machine);
    }
    ...
    /**
     * Gets the InventoryReport to look at in the next screen.
     * also in this function we check that the report exist and if
     * not exist showing the appropriate message in the screen
     * @param event the ActionEvent
     */
    @FXML
    void getInventoryReport(ActionEvent event) {
    	if(comboBoxYear.getValue()==null||comboBoxMonth.getValue()==null||comboBoxMachine.getValue()==null) {
    		ErrTXT.setText("*Error: Please choose year and month and machine!");
        return;
    	}
    	...
    	repo.setDeviceID(Integer.parseInt(comboBoxMachine.getValue()));
    	repo.setType("inventory");
    	repo.setRegion(((Employee) ChatClient.userloginController.getUser()).getRegion());
    	...
    	if(rMessage.equals("get Report Success"))
    	{
    		...
    		scene.changeScreen(new Stage(),"/clientGUI/ViewSpecificInventoryReport.fxml");
    	}
    }
